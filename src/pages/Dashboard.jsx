        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="flex flex-wrap gap-4 pt-8">
              <Link
                to="/assets"
                className="btn bg-gold-500 hover:bg-gold-600 text-black font-black border-none px-8 shadow-[0_10px_30px_-10px_rgba(239,183,8,0.5)] transition-all hover:-translate-y-1 active:scale-95 uppercase tracking-wider text-xs"
              >
                <ImageIcon className="w-4 h-4 mr-2" />
                Browse Gallery
              </Link>
              <Link
                to="/assets?action=upload"
                className="btn bg-white hover:bg-gray-100 text-black font-black border-none px-8 transition-all hover:-translate-y-1 active:scale-95 uppercase tracking-wider text-xs"
              >
              ...
              <div className="hidden xl:flex items-center gap-4 bg-gold-500/10 border border-gold-500/20 p-4 rounded-2xl backdrop-blur-xl shadow-2xl animate-pulse-gold max-w-sm">
                ...
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-gold-500 uppercase tracking-widest">
                    Vision of the Month
                  </p>
                  <p className="text-xs text-white/90 font-medium italic leading-relaxed">
                    "Quality is not an act, it is a habit. Let's build
                    excellence together."
                  </p>
                </div>
              </div>
          ...
          <div className="lg:col-span-5 grid grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-2xl bg-gold-500/10 border border-gold-500/20 backdrop-blur-md">
              ...
              <p className="text-[10px] font-bold text-gold-500 uppercase tracking-widest leading-none mb-1">
                Creative Report
              </p>
        ...
        <div className="flex items-center gap-12 animate-scroll-text whitespace-nowrap">
          {[
            ...
            <div key={i} className="flex items-center gap-4">
              <span className="text-[10px] font-black text-gold-500 tracking-[0.3em] uppercase">
                {text}
              </span>
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h3 className="text-4xl font-black text-white mb-2 group-hover:scale-105 transition-transform origin-left">
                    {stat.value}
                  </h3>
                ...
                <div
                  className={`p-3 rounded-xl ${stat.bgColor} ${stat.color} group-hover:scale-110 transition-transform duration-500`}
                >
            ...
            <div className="space-y-3">
              {stats?.upcomingFestivals?.length > 0 ? (
                stats.upcomingFestivals.map((f) => (
                  <div
                    key={f.id}
                    className="group p-4 rounded-2xl bg-black/20 border border-white/5 hover:bg-white/5 transition-all cursor-pointer"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[10px] font-bold bg-rose-500/10 text-rose-500 px-2 py-1 rounded border border-rose-500/10 uppercase">
                        {f.category}
                      </span>
                      <span className="text-[10px] font-bold text-muted-foreground flex items-center gap-1">
                        <Clock className="w-3 h-3 group-hover:text-gold-500 transition-colors" />{" "}
                        {f.daysLeft} days left
                      </span>
                    </div>
                    <h4 className="font-bold text-sm text-foreground mb-1 group-hover:text-gold-500 transition-colors">
                      {f.title}
                    </h4>
                    <p className="text-[10px] text-muted-foreground font-medium">
                      Coming on {new Date(f.date).toLocaleDateString()}
                    </p>
                  </div>
                ))
              ) : (
              ...
              {[
                {
                  label: "New Poster",
                  icon: FileText,
                  link: "/assets/poster?action=upload",
                },
                {
                  label: "ID Cards",
                  icon: User,
                  link: "/assets/id_card?action=upload",
                },
                {
                  label: "Upload Logo",
                  icon: ImageIcon,
                  link: "/assets/logo?action=upload",
                },
                { label: "Team", icon: Users, link: "/users" },
                {
                  label: "Visiting Card",
                  icon: Contact2,
                  link: "/assets/visiting_card?action=upload",
                },
                {
                  label: "T-Shirt",
                  icon: Shirt,
                  link: "/assets/tshirt?action=upload",
                },
                {
                  label: "Banner",
                  icon: Flag,
                  link: "/assets/banner?action=upload",
                },
                {
                  label: "Envelope",
                  icon: Mail,
                  link: "/assets/envelop?action=upload",
                },
              ].map((action, idx) => (
                <Link
                  key={idx}
                ...
                >
                  ...
                  <span className="text-xs font-bold text-muted-foreground group-hover:text-white uppercase tracking-wider">
                    {action.label}
                  </span>
                </Link>
              ))}
